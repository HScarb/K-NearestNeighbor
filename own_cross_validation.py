...
def splitDataSet(fileName, split_size,outdir):
    '''
    用来切分数据集，如果是十折交叉，则split_size取10，filename为整个数据集文件，outdir则是切分的数据集的存放路径。
    :param fileName:    整个数据集文件
    :param split_size:
    :param outdir:      切分的数据集的存放路径
    :return:
    '''
...
def generateDataset(datadir,outdir): #从切分的数据集中，对其中九份抽样汇成一个,
    ...
    '''
    从切分的数据集中留出一份作为测试集（无需抽样），对其余的进行抽样然后合并为一个作为训练集
    :param datadir:
    :param outdir:
    :return:
    '''
...
if __name__ == '__main__':
    os.chdir("C:\WorkSpace\Python\K-NearestNeighbor")               #你的数据存放目录
    datadir = 'split10_1'                   #切分后的文件输出目录
    splitDataSet('data.txt',10,datadir)     #将数据集datasets切为十个保存到datadir目录中
    #==========================================================================
    outdir = 'data.txt'	                #抽样的数据集存放目录
    ...
    clfname = "RF_1"
    #==========================================================================
    curdir = "C:\WorkSpace\Python\K-NearestNeighbor" #工作目录
